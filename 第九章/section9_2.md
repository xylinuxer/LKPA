...
&emsp;&emsp;由于设备种类繁多，相应的设备驱动程序也非常之多。尽管设备驱动程序是内核的一部分，但设备驱动程序的开发往往由很多人来完成，如业余编程高手、设备厂商等。为了让设备驱动程序的开发建立在规范的基础上，就必须在驱动程序和内核之间有一个严格定义和管理的接口，例如SVR4提出了DDI/DDK规范，其含义就是设备与驱动程序接口／设备驱动程序与内核接口（Device-Driver  
Interface／Driver-Kernel Interface）。通过这个规范，可以规范设备驱动程序与内核之间的接口。
...
&emsp;&emsp;(2) 通过cat /proc/devices  
